...
#     新 key（如 lmfp_exp2）即可。脚本不会自己写文件——配置登记永远是
#     人看一眼、亲手贴进去，防止坏数据混进仓库。
#
# 命名约定：key 用"本批实验材料简写_exp编号"风格（如 lmfp_exp1、lmfp_exp2）。
# 注意 key 跟实验批次走、不跟校准标样走——标样只是校准用的"尺子"。
# 隐私注意：label 只写批次级信息，不写数据集运行号 / 未公开样品名等
# 实验细节（仓库要公开）。LaB₆ 是公开标样（NIST SRM 660），可以提。
CONFIGS = {
    # ── lmfp_exp1 ──
    # 本条目 = lmfp 批次实验的几何。LaB₆ 标样是当时专门放进去做几何
    # 校准的样品，校准完的这套几何供整个 lmfp 批次使用——所以 key 跟
    # 批次材料走，不跟标样走。
    # 校准细节：手动初值圆心 (1024, 1024)，3 次精修取平均。距离对初值
    # 不敏感（1595.80 mm，与自动定位初值的结果一致）；PONI/倾斜角与
    # 自动定位初值存在近似简并、两组解残差相当——统一采用手动初值解，
    # 自动定位（find_ring_center）仅作新数据的便捷初值。
    "lmfp_exp1": {
        # 给人看的备注（中文）：这组几何是哪批实验、怎么标定出来的
        "label": "lmfp 批次（LaB₆ 标样标定）",
...
def get_config(name=None):
    ...
    参数：
        name : str 或 None
            条目 key（如 "lmfp_exp1"）；None → 取 DEFAULT_CONFIG
