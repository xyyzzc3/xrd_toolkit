...
#     新 key（如 lmfp2_lab6）即可。脚本不会自己写文件——配置登记永远是
#     人看一眼、亲手贴进去，防止坏数据混进仓库。
#
# 命名约定：key = 材料简写 + 批次编号 + 标样简写（lmfp1_lab6 = lmfp 第 1 批、
# LaB₆ 标样标定）——以批次为主、后缀校准用的"尺子"，一眼看出归属和溯源。
# 隐私注意：label 只写批次级信息，不写数据集运行号 / 未公开样品名等
# 实验细节（仓库要公开）。LaB₆ 是公开标样（NIST SRM 660），可以提。
CONFIGS = {
    # ── lmfp1_lab6 ──
    # 本条目 = lmfp 第 1 批实验的几何。LaB₆ 标样是当时专门放进去做几何
    # 校准的样品，校准完的这套几何供整个 lmfp 批次使用——所以 key 以
    # 批次材料为主（lmfp1），后缀标样（lab6）方便溯源。
