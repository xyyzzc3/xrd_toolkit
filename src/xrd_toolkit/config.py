...
# 任务三对 LaB₆ 标样校准得到的几何参数：自动定位圆心（find_ring_center）
# 作初值，3 次精修取平均（2026-09-13）。距离对初值不敏感（1595.80 mm，
# 与手动初值的结果一致）；PONI/倾斜角与手动初值存在近似简并、两组解
# 残差相当——统一采用自动圆心约定，因为新数据没有手动圆心可用，
# 自动定位是全项目唯一可复现的约定。
...
CALIBRATED = dict(
    ...
    poni1_m=1033.57 * 200e-6,     # PONI 横向（米）
    poni2_m=985.50 * 200e-6,      # PONI 纵向（米）
    rot1_deg=-0.2648,             # 倾斜角 1（度）
    rot2_deg=-0.0815,             # 倾斜角 2（度）
)

# 环圆心 = 直射束落点 B（像素坐标，任务三校准结果）。
# 注意：B ≠ PONI（法线垂足）——探测器有倾斜时两者差 ~39 px。
