...
#         比例，跌破 80% 的位置（LMFP/lab6 两套数据实测同为 7.438°）；
...
#   实测核对（LMFP_1_atten0-00029）：(200) 算 1.350° vs 实测 1.363°、
#   (101) 算 1.631° vs 实测 1.637°、(002) 算 2.970° vs 实测 2.987°，
#   低角度主要峰全部吻合（Δ < 0.02°），说明这套参数可信。
MATERIALS = {
    "lab6": {"kind": "calibrant"},
...
def detect_halo_end(tth, curve, plateau_lo=0.5, plateau_hi=1.0,
    ...
    是平台之后的第一次明显"爬坡"。做法：
      ...
      2) 平台高度 = [plateau_lo, plateau_hi] 里曲线的中位数
         （本仪器两个样品的晕区平台都在 0.5~1.0°，见 lab6-00024 实测）；
...
def detect_failure_end(tth, data, hi_geom, live_threshold=0.8):
    ...
    第一个跌破 live_threshold(80%) 的位置即失效点（LMFP/lab6 实测
...
def geometric_failure_point(poni_px, image_shape, pixel_size_m, dist_m,
    ...
    （与 detect_failure_end 在 LMFP/lab6 上的数据驱动结果 7.438°
