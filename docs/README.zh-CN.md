...
- **工具栏五个入口**——`[校准] [1D] [扣背景] [对比] │ [绘图]`：点一个 = 右侧参数面板翻到那一页，只放本阶段的东西（校准：几何 + 校准表单；1D：积分设置 + [出 1D 图]；扣背景：扣除控件 + [重画]；对比：归一化/配色 + 热图显示 + [出对比][出热图]；绘图：六个类型选择 + 显示参数 + [出图（勾选文件）][只重画当前][导出图片…]）。**用的最多的四个放外面，其余的要时再进绘图页找**（用户 2026-09-24 定稿）。
...
- **分阶段产物缓存**——算过的 1D 曲线落盘（`outputs/_stage/`，本地不进 git），**跨会话**再分析同一批数据时直接复用、不用重积分（实测 3 张：1.4 s → 0.4 s；81 张推算 38 s → 10 s）。缓存键 = 文件指纹（名字+大小+修改时间+头部哈希）+ 几何条目+ 2θ 范围 + 点数 + **积分实现版本号**（算法改了旧产物自动作废）；命中会在日志里写明「复用缓存：…」（不许静默复用）；「绘图」页有 [清空缓存]。**扣背景也有一类产物**：在「扣背景」页点 [批量扣背景（勾选文件）]，把当前图上的锚点用到整批——**锚点只传 2θ 位置，强度到每张图自己的曲线上重新取**（一批数据的背景形状是共同的、绝对强度不是，直接套会把别人的基线抬错几倍）；扣完各存一份，**对比 / 热图默认优先读它**（日志写明「扣背景产物 N 条」，用哪一份看得见）。改背景参数不会失效重算：产物键里含设置哈希，改了就当场重画（毫秒级），设置没变才读产物。
