...
- **分阶段产物缓存**——算过的 1D 曲线落盘（`outputs/_stage/`，本地不进 git），**跨会话**再分析同一批数据时直接复用、不用重积分（实测 3 张：1.4 s → 0.4 s；81 张推算 38 s → 10 s）。缓存键 = 文件指纹（名字+大小+修改时间+头部哈希）+ 几何条目+ 2θ 范围 + 点数 + **积分实现版本号**（算法改了旧产物自动作废）；命中会在日志里写明「复用缓存：…」（不许静默复用）；「绘图」页有 [清空缓存]。扣背景**不落盘**——它是实时可调的，冻成产物反而慢。
