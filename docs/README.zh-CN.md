...
<table>
  ...
  <tr>
    <td align="center" colspan="2">
      ...
      <sub>校准页（LaB₆ 标样）——当前配置与两个对比位 A / B，三列各显示距离 / 环心 / 环位偏差，下面是相对基准的 Δ 行与结论行。这里跑了两步：① 自动定位（结果记作「原始1」，环位偏差 0.26 px）→ ③ 在当前配置上再精修（「精修1」，0.23 px）——新批次的第一条结果按规则**直接采纳**，结论行写明来由（数字取自这一次运行，pyFAI 精修每次会略有不同）</sub>
    </td>
  </tr>
</table>

- **独立图面板**——每张图是一个 MDI 子窗口，互不牵连：可自由缩放（每次拖动记住这张图自己的画布比例）、弹出成独立系统窗口、按各自比例横排/竖排平铺；绘图区支持 Ctrl+滚轮缩放（Excel 式）。
- **工具栏五个入口**——`[校准] [1D] [扣背景] [对比] │ [绘图]`：点一个 = 右侧参数面板翻到那一页，只放本阶段的东西（校准：几何 + 校准表单；1D：积分设置 + [出 1D 图]；扣背景：扣除控件 + [重画]；对比：归一化/配色 + 热图显示 + [出对比][出热图]；绘图：六个类型选择 + 显示参数 + [出图（勾选文件）][只重画当前][导出图片…]）。**用的最多的四个放外面，其余的要时再进绘图页找**（用户 2026-09-24 定稿）。**开界面时上面一个都不选、右侧参数栏是收起的**（用户 2026-09-25 定）：选到哪个入口才露出哪一页的参数；没选过入口时从校准退出来也回到这个状态。
- **一键出图**——「绘图」页里的 [2D] [剖面] [1D] [瀑布] 四个类型按钮把勾选的文件一次全部出图；[对比] 把多条 1D 曲线叠进同一面板，四种归一化模式（各曲线最强峰 / 全体最强峰 / 指定文件 / 不归一化，默认不归一化）。四个视图全部接线：2D 显示原始衍射图（对数强度 magma 色图、带颜色条、自动或手动对比度、束心十字），剖面沿过束心的直线按设定角度采样，1D 是全体方位角积分，瀑布把 36 个扇形各自积分后按 χ 错开堆叠（行标签 = 扇区 χ，每条曲线画到探测器截断处）。
...
- **分阶段产物缓存**——算过的 1D 曲线落盘（`outputs/_stage/`，本地不进 git），**跨会话**再分析同一批数据时直接复用、不用重积分（实测 3 张：1.4 s → 0.4 s；81 张推算 38 s → 10 s）。缓存键 = 文件指纹（名字+大小+修改时间+头部哈希）+ 几何条目+ 2θ 范围 + 点数 + **积分实现版本号**（算法改了旧产物自动作废）；命中会在日志里写明「复用缓存：…」（不许静默复用）；「绘图」页有 [清空缓存]。**扣背景也有一类产物**：在「扣背景」页点 [批量扣背景（勾选文件）]，把当前图上的锚点用到整批——**锚点只传 2θ 位置，强度到每张图自己的曲线上重新取**（一批数据的背景形状是共同的、绝对强度不是，直接套会把别人的基线抬错几倍）；扣完各存一份，**对比 / 热图默认优先读它**（日志写明「扣背景产物 N 条」，用哪一份看得见）。改背景参数不会失效重算：产物键里含设置哈希，改了就当场重画（毫秒级），设置没变才读产物。**产物在文件栏里看得见**（用户 2026-09-25 定）：文件栏是一棵树，顶上是「原始数据」组，往下是各阶段分组——「1D 产物」= 当前设置下已经算好的；「扣背景 09-25 14:03（锚点 5 个，窗口 2°）…」= 每次 [批量扣背景] 一组（组名带时间与参数，一眼看得出是哪一套参数的结果）。**勾组 = 组里整组勾上**（半勾表示只勾了一部分），整组勾上去 [对比]/[热图] 就是把那一批结果叠成一张图；产物条目出 1D 图 / 导出走的也是那一份产物（不重算，日志写明"直接读盘不重算"）；**两类产物待遇不同**：「1D 产物」就是那条原始积分曲线（只是钉在某一份缓存上），跟原始文件一个待遇——锚点、自动基线照用，也能整组进 [批量扣背景]（扣出来的结果挂在*那一份 1D 的键*下面：勾的是哪一条就扣哪一条）；「扣背景产物」才是"已完成"的，面板上强制「不扣」、也不再被扣一遍（再扣就是二次相减）；产物分组上右键可以**删掉这一组**（台账与盘上的产物一起清）。分组只在"有事发生"时重建（导入/删除、批量扣背景、清空缓存），不是定时刷新。
...
- **每面板一行标题栏**——面板顶上一行就是它全部的壳：Home（回到这张图本来的样子：缩放/平移不算，重算才刷新）/ 放大镜 / Customize（标题、轴标签、纵轴线性/对数、图边距）/ 保存 PNG + [弹出]/[关闭]。按住这一行拖 = 移动面板、双击 = 占满绘图区（原生标题栏被换成自绘的一行，整块壳从 83 px 瘦到 26 px，同样的面板高度里画布多拿 57 px）；弹出成独立窗口时系统标题栏回来管窗口管理，这一行继续提供按钮，但**不显示名字**（数据集已经出现在参数坞的「编辑对象」和图上标题里，鼠标悬停这一行也能看到是哪块），而**当前编辑对象**的那块用更深的底色标出来（活动/非活动的老惯例）。
- **校准功能**——[校准] 开关打开校准页：一张**三列表**（**当前配置** + 两个对比位 **A** / **B**，三列都可以从"跑过的东西"里选）压着两个动作块——**自动**（定位环心并精修 / 在当前配置上再精修）与**手动**（图上点环：≥3 点、覆盖 ≥2 环、±0.5° 自动吸附）。当前配置一开始是**从选中条目借来的原始几何**；[编辑…] 可以换成别的条目预填或直接手输，**只要手改过就标成"自定义"**、之后不被自动替换。**每个结果都按自己的名字累积**（原始 / 自动1 / 手动1 / 精修1 …），并按 A→B→A 轮换填进对比位——除非你手动选过那个槽（选过就**钉住**，新结果不再覆盖它）。**"用的到底是哪一份"由引擎指标决定**：结果要在环位偏差上赢过当前配置 **0.05 px** 才替换（同一张图重复跑，几何本身抖 PONI 1.6~2.2 px 而环位只抖 0.014~0.029 px，比这更小的差是跑动噪声）。任意两列都能对比：选一个**基准**（当前配置 / A / B），读 Δ 行（距离与环位偏差）+ 结论行。PONI 与倾斜角标 ⚠——它们与距离/波长退化，差异大不等于更准；自洽残差**不入表**（它量的是迭代收敛，不是准不准）。出口从不藏着：进入后工具栏开关文字变为「退出校准」，校准页底部还有 [返回分析模式] 按钮一键翻回常规参数面板。校准结果可 [保存为配置] 存成命名条目（本地 config_user.json，不进 git），保存后立即出现在"几何配置"下拉框并自动选中，重启仍在，命令行脚本同样可用 `--config` 选取。图上那圈青线是**当前几何预测的真实环**：每个点都由 pyFAI 的 2θ 场反解而来，倾斜探测器上就以它本来的椭圆样子画出来——用"圆心 + 半径"的正圆近似会整体偏 8~23 px（环的公共圆心是直射束落点，不是 PONI）。点图判环用同一套几何，因此点你看到的那条线不会判成隔壁环号；若当前几何把 16 条环全推出探测器（距离/像素尺寸/波长填错或精修跑出离谱解），面板会在日志里明说并把视野放大到看得见，而不是默默画一圈看不见的线。每次校准还会在日志里附上**引擎指标**：环位偏差中位（px，括号里是精修前初值的同一数字）、16 条环里完整跑出来的条数、以及逐环反推的 LaB₆ 晶格常数离散度。它们量的是拟合残差量不到的东西——实测过残差 0.0000°（看着完美）而环位偏 9.6 px 的情况（故意喂一组偏离真环的点），因为残差是迭代自洽量、不是精度指标。环位偏差抓"整体几何错"，晶格常数离散度抓"某些环与单一几何不符"（环号认错、探测器畸变、某条峰被顶偏）；距离与波长在标样上一阶简并，所以距离误差几乎不推动离散度，别用它判断距离。新批次从「当前配置」出发：[编辑…] 里可借另一条条目预填或直接手输，也可用 [加载参数] 导入 .poni。**像素尺寸必须确认过才允许跑校准**，而且只有**像素值真的变了**才会再次要求确认。指标抓不到像素填错：距离、波长、像素尺寸只以**乘积**进入计算，环照样严丝合缝，但报出来的距离（以及由它推出的东西）整体错同一个比例。[保存为配置] 保存的就是**当前配置**——借来的、手输的、采纳来的都行——并写入血缘（`derived_from`、`method`、`created`）。分析页与几何有关的部分已改为**只读**：只负责选条目 + 灰色摘要显示几何；[加载参数]/[保存参数]/[删除] 三个按钮搬到了校准页。
- **批量管线**——[打开文件夹] 一键遍历目录（.tif/.tiff/.edf/.cbf 全收，重复自动跳过；把文件夹直接拖进窗口同样生效），任意视图按钮对全部勾选文件一次出图，日志与状态栏进度条同时报进度（完成行末尾的（k/n）；开面板本身每 8 张报一行、期间界面不闷住；**大批量自动合并日志**——开面板一行汇总、每 8 张一行进度、收尾一行带用时与复用缓存张数，81 张不再刷 162 行）；**一次批量最多画前 24 张**（按文件列表顺序）——开图是唯一随数量变慢的成本（第 1 张 136 ms、第 100 张 287 ms），每张还占 ≈15 MB，超出的**照样在后台算完并落盘**（只算不画）：之后单独点开就是复用缓存、秒开，日志会写清楚；要看整批一眼，用 [热图]/[对比] 一张图放完；[导出数据] 把每个文件的 1D 结果存成两列 `integrated_2th.txt` / `.chi`（`outputs/{文件名}/` 下，与命令行逐字同格式），可顺带生成 `1d_summary.csv` 总表（每文件一列强度；2θ 网格不一致时弹窗三选：取公共交集重插值 / 跳过范围不同的文件 / 取消）。单个文件失败不中断整批。**导入默认一个都不勾**（用户 2026-09-25 定）：200 张数据要自己说了算，勾选用文件栏上方那排 [全选] / [全不选] / [按条件选…]——按条件选支持**区间**（第几个到第几个）、**间隔**（每 N 个选 1 个，可带起始偏移）、**名字包含**筛选，三者可叠加（名字筛选与区间/间隔是"与"关系），还可以"追加到当前选择"跨几段攒一批；弹窗里有实时预览（将选中 N 个文件）。
...
> offscreen 的 GUI 套件曾经在一个进程里面板建够多之后会死锁（两个机制：matplotlib 工具栏在自己的构造函数里递归；以及 PySide6 侧的一次锁序反转——主线程握 GIL 等一把 Qt 内部锁，而工作线程持着那把锁、在销毁每任务线程时要 GIL）。**两个都已修**：面板工具栏改成自绘、后台任务改用长驻工作线程；`python scripts/stress_panels.py` 就是当初挂死的那个压力探针，现在是回归检查。`python scripts/run_tests.py` 仍然给套件带看门狗：万一再卡住，它会打印所有线程的 Python 栈并以非 0 退出，不会静默挂到天荒地老。
