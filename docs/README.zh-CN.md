...
- **保存/加载几何参数（.poni）**——"几何配置"下拉框旁的 [保存参数] 把当前选中配置写成 pyFAI 通用交换格式 .poni 文件（保存内容：探测器距离、中心点 poni1/poni2 米制坐标、像素尺寸、波长、倾斜角 rot1/rot2；作业规格中的掩膜文件为可选项——引擎尚未支持掩膜且 .poni 格式本身无掩膜字段，故不写）；[加载参数] 读 .poni 存成命名配置条目（与 [保存为配置] 同源，自动选中、重启仍在）。标定一次到处用，别的工具标定的几何直接拿来用，避免每次重新校准。
