...
- **批量管线**——[打开文件夹] 一键遍历目录（.tif/.tiff/.edf/.cbf 全收，重复自动跳过；把文件夹直接拖进窗口同样生效），任意视图按钮对全部勾选文件一次出图，日志实时报进度（完成行末尾的（k/n）；开面板本身每 8 张报一行、期间界面不闷住）；[导出数据] 把每个文件的 1D 结果存成两列 `integrated_2th.txt` / `.chi`（`outputs/{文件名}/` 下，与命令行逐字同格式），可顺带生成 `1d_summary.csv` 总表（每文件一列强度；2θ 网格不一致时弹窗三选：取公共交集重插值 / 跳过范围不同的文件 / 取消）。单个文件失败不中断整批。
