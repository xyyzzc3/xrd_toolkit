...
- **每面板一行标题栏**——面板顶上一行就是它全部的壳：Home / 放大镜 / Customize（标题、轴标签、纵轴线性/对数、图边距）/ 保存 PNG + [弹出]/[关闭]。按住这一行拖 = 移动面板、双击 = 占满绘图区（原生标题栏被换成自绘的一行，整块壳从 83 px 瘦到 26 px，同样的面板高度里画布多拿 57 px）；弹出成独立窗口时系统标题栏回来管窗口管理，这一行继续提供按钮，但**不显示名字**（数据集已经出现在参数坞的「编辑对象」和图上标题里，鼠标悬停这一行也能看到是哪块），而**当前编辑对象**的那块用更深的底色标出来（活动/非活动的老惯例）。
