...
- **校准工作台**——三列表（当前配置 + 对比位 A / B）压着自动与手动两套动作；每个结果按自己的名字累积，"用的到底是哪一份"由引擎指标决定。参数面板顶部钉着两行全局控件：编辑对象，以及当前用哪条几何条目（悬停看像素尺寸 / 波长 / 距离）——校准页上它置灰，并在那里给出 [返回分析模式] 出口。指标**抓不到什么**也写在 [详细说明](NOTES.zh-CN.md#校准) 里。
