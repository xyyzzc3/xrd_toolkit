...
<table>
  ...
  <tr>
    <td align="center" colspan="2">
      ...
      <sub>校准工作台（LaB₆ 标样）——三步流程（① 自动定位精修 / ② 可选手动点环 / ③ 在当前几何上再精修），任选两个来源的 A | B | Δ 对比，以及被选中的"当前使用"；此例 ③ 只把环位偏差改善了 0.0028 px（小于 0.05 px 门槛），日志因此保持 ① 为当前使用</sub>
    </td>
  </tr>
</table>
...
- **校准模式**——[校准] 开关切到校准工作台，走**三步流程**：① 一键定位环心并精修 → ② 可选：在图上点环做手动解（≥3 点、覆盖 ≥2 环、±0.5° 自动吸附）→ ③ 可选：在当前几何上再精修一遍。每一步各留一份结果，**"当前使用"由引擎指标决定**：后来的来源要在环位偏差上赢过当前 **0.05 px** 才替换（同一张图重复跑，几何本身抖 PONI 1.6~2.2 px 而环位只抖 0.014~0.029 px，比这更小的差是跑动噪声）。任意两个来源可并排对比（A | B | Δ + 结论行）；PONI 与倾斜角标 ⚠——它们与距离/波长退化，差异大不等于更准；自洽残差**不入表**（它量的是迭代收敛，不是准不准）。出口从不藏着：进入后工具栏开关文字变为「退出校准」，校准页底部还有 [返回分析模式] 按钮一键翻回常规参数面板。校准结果可 [保存为配置] 存成命名条目（本地 config_user.json，不进 git），保存后立即出现在"几何配置"下拉框并自动选中，重启仍在，命令行脚本同样可用 `--config` 选取。图上那圈青线是**当前几何预测的真实环**：每个点都由 pyFAI 的 2θ 场反解而来，倾斜探测器上就以它本来的椭圆样子画出来——用"圆心 + 半径"的正圆近似会整体偏 8~23 px（环的公共圆心是直射束落点，不是 PONI）。点图判环用同一套几何，因此点你看到的那条线不会判成隔壁环号；若当前几何把 16 条环全推出探测器（距离/像素尺寸/波长填错或精修跑出离谱解），面板会在日志里明说并把视野放大到看得见，而不是默默画一圈看不见的线。每次校准还会在日志里附上**引擎指标**：环位偏差中位（px，括号里是精修前初值的同一数字）、16 条环里完整跑出来的条数、以及逐环反推的 LaB₆ 晶格常数离散度。它们量的是拟合残差量不到的东西——实测过残差 0.0000°（看着完美）而环位偏 9.6 px 的情况（故意喂一组偏离真环的点），因为残差是迭代自洽量、不是精度指标。环位偏差抓"整体几何错"，晶格常数离散度抓"某些环与单一几何不符"（环号认错、探测器畸变、某条峰被顶偏）；距离与波长在标样上一阶简并，所以距离误差几乎不推动离散度，别用它判断距离。新批次从「校准初值」区出发：初值可借某条已有条目、可手输（像素·波长·距离），也可导入 .poni；**像素尺寸必须显式勾选确认才允许跑校准**——像素填错时整幅几何都会错，而结果看起来很正常。校准页保存的条目会带上血缘（`derived_from`、`method`、`created`）。分析页与几何有关的部分已改为**只读**：只负责选条目 + 灰色摘要显示几何；[加载参数]/[保存参数]/[删除] 三个按钮搬到了校准页。
...
│   ├── services/ring_metrics.py # 校准质量指标：环覆盖 / 环位偏差 / 晶格常数一致性（无 Qt）
│   └── gui/                    # PySide6 桌面界面（python -m xrd_toolkit.gui）
├── tests/                      # 合成图像单元测试（unittest）：部分环几何 + 自研积分回归护栏 + 引擎指标标定
