...
**参数进出。** 新批次从「当前配置」出发：[编辑…] 里可借另一条条目预填或直接手输，也可用 [加载参数] 导入 .poni。[保存为配置] 保存的就是**当前配置**——借来的、手输的、采纳来的都行——并写入血缘（`derived_from`、`method`、`created`），存成命名条目（本地 config_user.json，不进 git）：立即出现在"几何配置"下拉框并自动选中，重启仍在，命令行脚本同样可用 `--config` 选取。几何配置下拉框自己住在**参数面板顶部钉住的一行**里，五个入口页都看得见；悬停它就能看到当前条目的像素尺寸 / 波长 / 距离。分析页与几何有关的部分是**只读**的：只负责选条目；校准页上这一行置灰（校准用的是那一页自己的「当前配置」），并在那里多一个 [返回分析模式] 出口。[加载参数]/[保存参数]/[删除] 三个按钮也在校准页。[保存参数] 把当前选中配置写成 pyFAI 通用交换格式 `.poni`（探测器距离、中心点 poni1/poni2 米制坐标、像素尺寸、波长、倾斜角 rot1/rot2；掩膜文件为可选项——引擎尚未支持掩膜且 .poni 格式本身无掩膜字段，故不写）；[加载参数] 读 .poni 存成命名条目（与 [保存为配置] 同源）。导入或保存的用户条目可用 [删除] 移除（内置条目置灰不可删）：删除前弹确认框，删后自动回退默认条目。
