...
- **自动基线**——滑动窗估计背景水平，窗口宽度应取最宽峰宽的 3~10 倍（改成"窗口内直线拟合"之后可用范围从"1° 附近"放宽到 0.5~3°）。**默认 0.5°**（2026-09-26 从 1.0° 改）：LMFP 数据上 1° 窗口会把 1.6~2.6° 的宽非晶鼓包当结构留下（残留中位 34、最大 527），0.5° 能扣掉（残留中位 −1、最大 322）而巨峰高度几乎不变（2510→2516）；0.1° 也能扣掉，但骑峰更明显（峰下抬高 211 vs 142）；
- **手动锚点**——在 1D 图上点选"只有背景"的位置，基线连过这些点、两端按首末锚点线性外推；锚点之间默认走**保单调平滑**（pchip：过点、光滑、不过冲），另有折线（实验室惯例）与自然样条（可能过冲）可选。

**自动基线的两阶段与"局部线性"。** 阶段 1 取窗口内的低分位（一个不会被峰抬高、但偏低的下限），阶段 2 掩掉峰点后给出**窗口中心处的水平**——这一步 2026-09-26 从"掩峰后取均值"改成"**窗口内直线拟合**"：陡降段上掩峰会掩掉窗口高的一侧，均值因此落到真值以下，低角系统性扣过头（合成真值实测 −139/−163/−533，窗口 1°/2°/3°），局部线性没有这个偏差（同口径 +3.7/+1.0/−9.3，平均绝对偏差 42→35、63→46、94→67）。实现是向量化的（掩膜是逐点判据 → 前缀和 + 二元正规方程，10 万点 21 ms）。
...
**批量。** [打开文件夹] 一键遍历目录（.tif/.tiff/.edf/.cbf 全收，重复自动跳过；把文件夹直接拖进窗口同样生效），任意视图按钮对全部勾选文件一次出图，日志与状态栏进度条同时报进度（完成行末尾的（k/n）；开面板本身每 8 张报一行、期间界面不闷住；**大批量自动合并日志**——开面板一行汇总、每 8 张一行进度、收尾一行带用时与复用缓存张数，81 张不再刷 162 行）。**勾选超过 24 张就一张都不画**（用户 2026-09-26 定，防爆图）——开图是唯一随数量变慢的成本（第 1 张 136 ms、第 100 张 287 ms），每张还占 ≈15 MB：这批全部走「只算不画」，算完落盘并进文件栏「1D 产物」，**双击条目看一张、右键分组 [打开整组] 一起看**（整组超过 24 张会先弹确认：每张 ≈15 MB）。24 张及以下照旧一次全画。要看整批一眼，用 [热图]/[对比] 一张图放完。[导出数据] 把每个文件的 1D 结果存成两列 `integrated_2th.txt` / `.chi`（`outputs/{文件名}/` 下，与命令行逐字同格式），可顺带生成 `1d_summary.csv` 总表（每文件一列强度；2θ 网格不一致时弹窗三选：取公共交集重插值 / 跳过范围不同的文件 / 取消）。单个文件失败不中断整批。
...
**颜色跟着它所属的那张图走。** 2D 面板的色图原先写死 magma；Customize 里现在给 2D 面板多一个「色图」下拉（按面板记进快照、点 [应用] 重画），和它本来就有的「对比逐曲线颜色」并排。分工一句话：**这张图现在的颜色**在 Customize，**新图默认用什么颜色**（曲线配色色板）在参数页。

参数坞顶层固定三行：编辑对象、几何配置、**数据参数（2θ 积分范围 + 输出点数）**。范围原先只在 1D 页，想给对比/热图换范围得先走回 1D 页（用户 2026-09-27：一 d 画图时能选范围，后面处理时没法选范围，比如对比时，参数里加上）——它们本来就是「参与计算的数据参数」，和几何配置同一类，所以并排住在坞顶、四个分析页都看得见——**校准页把它收起来**（用户随后指出「校准页参数放 2theta 范围干嘛」：校准在原始图上定几何，全程不读 2θ 区间）；几何配置那一行则相反，校准页要留着，它是借用起点，也是 [加载参数]/[保存参数]/[删除] 的对象。1D 页上的**视图 2θ 范围搬去了绘图页的「1D 显示」小节**——它本来就是显示参数，而在 1D 页它紧挨着坞顶那行积分范围、长得一模一样（用户当时的话：「1d 参数页两个范围。。。」）；现在 1D 页只剩下一行范围，两个含义各住各的地方：坞顶那行管积分（改了要重积分），视图范围现在**只能靠缩放/平移**（用户的决定：「显示范围用户自己放大就行了」），那对输入框已撤——参数键还在（缩放写回、[恢复默认] 复位），只是不再有控件显示（`_param_box_set` 见不到控件就跳过；`_display_snapshot` 会把没有控件的键从旧快照带过来，免得 [应用] 悄悄丢掉显式视图范围）。热图的显示控件（色图 / 对数强度 / 归一化）现在**改了立刻重画**——原先这几个控件谁都没接，画出来的热图根本改不了颜色。

**扣背景配方按文件记。** [批量处理] 原先只把模式/窗口写进「当时开着」的面板（锚点本来按文件存）：给 A 扣完、批量，再点开 B，它显示「不扣」——B 明明有产物。现在配方落进 `proc_recipes[path]`（每个目标都写，`anchor_source` 记着借的是哪个文件的锚点），开任一面板时按它回填快照：**处理过的文件点开就是扣过的样子**，处理页写着「锚点基准 — <文件名>」（手调过的记「本图手点」，也存配方）。同一思路往下走一层就是「自动 + 锚点校正」（用户 2026-09-27）：自动基线按你点的锚点做校正——锚点处的残差用保单调插值摊到全曲线，基线严格过你的点、形状仍是自动那份；所以锚点那行现在在 **auto** 模式也露出来，窗口默认也跟着调到 0.3°。

**产物在文件栏里分组。** 文件栏是一棵树：顶上是「原始数据」组，往下是各阶段分组——「1D 产物」= 当前设置下已经算好的；「扣背景 09-25 14:03（锚点 5 个，窗口 2°）…」= 每次 [批量扣背景] 一组（组名带时间与参数，一眼看得出是哪一套参数的结果）。热图的行名会缩成号段（"LMFP_1_atten0-00029.tif" → "00029"）并按面板高度抽稀——81 个长名字会糊成一条黑带还把图挤到右边；对比面板**超过 12 条曲线就不再画图例**（81 行名字比数据还占地方，认曲线改看状态栏悬停读数，圆点颜色跟曲线一致）；绘图区一角的 [全关] 一次关掉所有图面板（与单个 × 同一条路径，日志报一行汇总，含几张没存过盘）。

**勾组 = 组里整组勾上**（两态：全勾亮、其余一律空格——半勾那根横线跟空格分不清，用户 2026-09-26 让去掉），整组勾上去 [对比]/[热图] 就是把那一批结果叠成一张图。条目上**双击 = 打开这一张的 1D 图**，分组右键 = [打开整组 1D 图]（超过 24 张先确认）。右键可以**删产物**：分组上右键 = 删掉这一组（扣背景批次连台账一起清），条目上右键 = 只删这一条——**所有产物都能在文件栏里删**，「1D 产物」组也一样（它的删除入口就在这里，不改设置也能清）。分组只在"有事发生"时重建（导入/删除、批量扣背景、清空缓存），不是定时刷新。
