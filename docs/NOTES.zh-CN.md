...
参数坞顶层固定三行：编辑对象、几何配置、**数据参数（2θ 积分范围 + 输出点数）**。范围原先只在 1D 页，想给对比/热图换范围得先走回 1D 页（用户 2026-09-27：一 d 画图时能选范围，后面处理时没法选范围，比如对比时，参数里加上）——它们本来就是「参与计算的数据参数」，和几何配置同一类，所以并排住在坞顶、每一页都看得见。热图的显示控件（色图 / 对数强度 / 归一化）现在**改了立刻重画**——原先这几个控件谁都没接，画出来的热图根本改不了颜色。
