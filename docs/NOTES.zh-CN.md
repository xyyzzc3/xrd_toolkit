...
- **自动基线**——滑动窗估计背景水平，窗口宽度应取最宽峰宽的 3~10 倍（改成"窗口内直线拟合"之后可用范围从"1° 附近"放宽到 0.5~3°）；
...
**产物在文件栏里分组。** 文件栏是一棵树：顶上是「原始数据」组，往下是各阶段分组——「1D 产物」= 当前设置下已经算好的；「扣背景 09-25 14:03（锚点 5 个，窗口 2°）…」= 每次 [批量扣背景] 一组（组名带时间与参数，一眼看得出是哪一套参数的结果）。**勾组 = 组里整组勾上**（两态：全勾亮、其余一律空格——半勾那根横线跟空格分不清，用户 2026-09-26 让去掉），整组勾上去 [对比]/[热图] 就是把那一批结果叠成一张图。条目上**双击 = 打开这一张的 1D 图**，分组右键 = [打开整组 1D 图]（超过 24 张先确认）。右键可以**删产物**：分组上右键 = 删掉这一组（扣背景批次连台账一起清），条目上右键 = 只删这一条——**所有产物都能在文件栏里删**，「1D 产物」组也一样（它的删除入口就在这里，不改设置也能清）。分组只在"有事发生"时重建（导入/删除、批量扣背景、清空缓存），不是定时刷新。
