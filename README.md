...
- **Processing page** — three optional steps, each redrawing as you change it: background subtraction (empty scan / auto baseline / manual anchors), **smoothing** (rolling average or Savitzky–Golay — SG keeps far more peak height at the same window, measured 82 % vs 91 % of the strongest peak), and **cut** (blanks one or more 2θ intervals so a giant peak stops squashing the rest). One click on [批量处理] applies the set to the whole batch, and each file's result becomes a 处理后 … group in the file bar that Compare / Heatmap / export then read.
