...
- **Five toolbar entrances** — `[校准] [1D] [扣背景] [对比] │ [绘图]`: clicking one turns the parameter dock to that stage's page, which carries only that stage's controls and its own produce button ([出 1D 图] / [重画] / [出对比] / [出图（勾选文件）] · [只重画当前] · [导出图片…]). The four most-used stages stay on the toolbar; everything else lives one click away in the Draw page.
