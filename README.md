...
- **File bar** — a tree: the raw files at the top, then one group per stage product (1D 产物, and one group per batch background run, named with its time and recipe). Checking a group checks everything in it, so a batch of background-subtracted curves goes to a comparison plot in two clicks. Imports start unchecked — select with [全选] / [全不选] / [按条件选…] (range, stride, name filter, with a live count).
